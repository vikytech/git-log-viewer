...
(function (e, t) {
    ...
    typeof environment == "object" && {}.toString.call(environment) === "[object Environment]" ? (typeof e == "undefined" ? r = {} : r = e.less = {}, i = r.tree = {}, r.mode = "rhino") : typeof e == "undefined" ? (r = exports, i = n("./tree"), r.mode = "node") : (typeof e.less == "undefined" && (e.less = {}), r = e.less, i = e.less.tree = {}, r.mode = "browser"), r.Parser = function (t) {
        ...
        return L.prototype = new Error, L.prototype.constructor = L, this.env = t = t || {}, this.optimization = "optimization"in this.env ? this.env.optimization : 1, p = {imports: v, parse: function (e, a) {
            ...
            var e, t, n = [], r, u, a, f = [];
            while ((a = w(this.extend)) || (t = w(this.element))) {
                a ? f.push.apply(f, a) : (f.length && x("Extend can only be used at the end of selector"), r = s.charAt(o), n.push(t), t = null);
                if (r === "{" || r === "}" || r === ";" || r === "," || r === ")")break
            }
            if (n.length > 0)return new i.Selector(n, f);
            f.length && x("Extend must be used to extend a selector, it cannot be used on its own")
        }, attribute: function () {
            var e = "", t, n, r;
            if (!w("["))return;
            ...
            if (match = /^([^@+\/'"*`(;{}-]*);/.exec(c[u]))return o += match[0].length - 1, new i.Anonymous(match[1])
        }, "import": function () {
            var e, n, r = o;
            m();
    ...
    (function (r) {
        ...
        e.Extend = function (t, n, r) {
            ...
            var t = [];
            for (d = 0; d < e.length; d++)t = t.concat(e[d].elements);
            this.selfSelectors = [
                {elements: t}
            ]
        }}
        ...
        var t = ["paths", "optimization", "files", "contents", "relativeUrls", "strictImports", "dumpLineNumbers", "compress", "processImports", "mime", "currentFileInfo"];
        e.parseEnv = function (e) {
            r(e, this, t), this.contents || (this.contents = {}), this.files || (this.files = {});
            if (!this.currentFileInfo) {
                var n = e.filename || "input";
                e.filename = null;
                var i = n.replace(/[^\/\\]*$/, "");
                this.currentFileInfo = {filename: n, relativeUrls: this.relativeUrls, rootpath: e.rootpath || "", currentDirectory: i, entryPath: i, rootFilename: n}
            }
        }, e.parseEnv.prototype.toSheet = function (t) {
            var n = new e.parseEnv(this);
            return n.href = t, n.type = this.mime, n
        };
        var n = ["silent", "verbose", "compress", "ieCompat", "strictMath", "strictUnits"];
        ...
        e.extendFinderVisitor = function () {
            ...
            for (i = 0; i < t.length; i++)for (s = 0; s < n.length; s++) {
                ...
                l = [h.selfSelectors[0]], o = f.findMatch(c, l), o.length && c.selfSelectors.forEach(function (t) {
                    a = f.extendSelector(o, l, t), newExtend = new e.Extend(h.selector, h.option, 0), newExtend.selfSelectors = a, a[a.length - 1].extendList = [newExtend], u.push(newExtend), newExtend.ruleset = h.ruleset, newExtend.parents = [h, c], h.firstExtendOnThisSelectorPath && (newExtend.firstExtendOnThisSelectorPath = !0, h.ruleset.paths.push(a))
                })
            }
            if (u.length) {
                this.extendChainCount++;
                if (r > 100) {
                    var p = "{unable to calculate}", d = "{unable to calculate}";
                    try {
                        p = u[0].selfSelectors[0].toCSS(), d = u[0].selector.toCSS()
                    } catch (v) {
                    }
                    throw{message: "extend circular reference detected. One of the circular extends is currently:" + p + ":extend(" + d + ")"}
                }
            ...
            if (e.root)return;
            var n, r, i, s = this.allExtendsStack[this.allExtendsStack.length - 1], o = [], u = this;
            for (i = 0; i < s.length; i++)for (r = 0; r < e.paths.length; r++) {
                selectorPath = e.paths[r];
                if (selectorPath[selectorPath.length - 1].extendList.length)continue;
                n = this.findMatch(s[i], selectorPath), n.length && s[i].selfSelectors.forEach(function (e) {
                    o.push(u.extendSelector(n, selectorPath, e))
                })
            }
